...
type AssignAwareContext = Context & {
  __assignOutput?: StateData;
};

function setAssignOutput(context: Context, output: StateData) {
  (context as AssignAwareContext).__assignOutput = output;
}

function consumeAssignOutput(context: Context): unknown {
  const assignAwareContext = context as AssignAwareContext;
  const output = assignAwareContext.__assignOutput;
  assignAwareContext.__assignOutput = undefined;
  return output;
}
...
function expressionReferencesForbiddenJsonataVariable(
  expression: string,
  forbiddenRef: string
): boolean {
  ...
  for (let index = 0; index <= expression.length - forbiddenRef.length; index += 1) {
    ...
    if (!expression.startsWith(forbiddenRef, index)) {
      continue;
    }
    ...
    const nextChar = expression[index + forbiddenRef.length];
...
function validateNoReservedStatesAccess(value: unknown, forbiddenRefs: string[]): void {
  ...
  for (const forbiddenRef of forbiddenRefs) {
    if (!expressionReferencesForbiddenJsonataVariable(expression, forbiddenRef)) {
      continue;
    }
...
export async function runState(
  ...
  while (true) {
    ...
    try {
      ...
      await applyStateAssignIfPresent(
        context,
        state,
        input,
        consumeAssignOutput(context) ?? output
      );
      return output;
...
const Executors: StateExecutors = {
  Pass: async (context, state, input) => {
    ...
    setAssignOutput(context, output);
  ...
  Parallel: async (context, state, input) => {
    ...
    setAssignOutput(context, outputData);
  ...
  Wait: async (context, state, input) => {
    ...
    await getRuntime(context).sleep(delay);
    debug('After delay');
    setAssignOutput(context, inputData);
  ...
  Choice: async (context, state, input) => {
    ...
    if (getQueryLanguage(context, state) === 'JSONata') {
      ...
      await applyChoiceRuleAssignIfPresent(context, state, matchedRule, inputData);
  ...
  Fail: async (context, state, input) => {
    ...
    const queryLanguage = getQueryLanguage(context, state);
    ...
    const errorName =
      queryLanguage === 'JSONata'
        ? typeof errorTemplate === 'undefined'
          ? defaultErrorName
          : String(
              await replacePathTemplateFields(
                errorTemplate,
                input,
                context,
                undefined,
...
async function applyChoiceRuleAssignIfPresent(
  ...
  await applyAssignRecord(context, state, matchedRule?.Assign, input);
}
...
async function applyStateAssignIfPresent(
  ...
  await applyAssignRecord(context, state, state.Assign, input, output);
}
...
async function applyAssignRecord(
  ...
  const assigned = await replacePathTemplateFields(
    ...
    context.StateEntryVariables,
    {
      queryLanguage: getQueryLanguage(context, state),
    }
  );
...
async function processMapState(context: Context, state: MapState, input: StateData) {
  ...
  if ('ItemReader' in state && state.ItemReader) {
    ...
    items = (await evaluateJsonataTemplateFields(
...
function resolveReaderMaxItems(
  readerConfig: unknown,
  input: StateData,
  context: Context
): number | undefined {
  if (!isRecord(readerConfig)) return undefined;
  if (typeof readerConfig.MaxItems === 'number') return readerConfig.MaxItems;
  if (typeof readerConfig.MaxItemsPath === 'string') {
    const selected = selectPath(readerConfig.MaxItemsPath, input, context);
    return typeof selected === 'number' ? selected : undefined;
  }
  return undefined;
}

function applyReaderMaxItems(
  readerConfig: unknown,
  items: unknown[],
  input: StateData,
  context: Context
): unknown[] {
  const maxItems = resolveReaderMaxItems(readerConfig, input, context);
...
async function processItemReader(
  ...
  if (itemReader.Resource === 'arn:aws:states:::s3:getObject') {
    ...
    if (inputType === 'CSV') {
      return {
        items: applyReaderMaxItems(
          readerConfig,
          parseCsvRecords(bodyString, readerConfig),
          input,
          context
        ),
      };
    ...
    if (inputType === 'JSONL') {
      return {
        items: applyReaderMaxItems(readerConfig, parseJsonLines(bodyString), input, context),
      };
    }
    ...
    return {
      items: applyReaderMaxItems(readerConfig, selected, input, context),
      source: undefined,
    };
  }
  ...
  if (itemReader.Resource === 'arn:aws:states:::s3:listObjectsV2') {
    ...
    if (transformation === 'NONE') {
      return {
        items: applyReaderMaxItems(readerConfig, listed, input, context),
        source: baseSource,
      };
    }
    ...
    if (transformation === 'LOAD_AND_FLATTEN') {
      ...
      const limitedFlattened = applyReaderMaxItems(
        readerConfig,
        flattened,
        input,
        context
      ...
      return {
        items: limitedFlattened.map(item => item.value),
        source: undefined,
        itemSources: limitedFlattened.map(item => item.source),
      };
    }
