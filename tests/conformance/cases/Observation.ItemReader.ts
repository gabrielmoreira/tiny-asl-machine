...
export const observationItemReaderCases: ConformanceCase[] = [
  ...
  customDefinitionCase({
    id: '012-listobjectsv2-none-maxitems-limits-materialized-items',
    title: 'ItemReader ListObjectsV2 with Transformation NONE respects MaxItems',
    group,
    tags: ['observation', 'itemreader', 'listobjectsv2', 'maxitems', 'aws_only'],
    definition: {
      StartAt: 'ReadItems',
      States: {
        ReadItems: {
    ...
    setupLocalResources: buildLocalItemReaderResources,
    expected: expectOutput([
      { Source: `s3://${itemReaderBucket}`, Key: `${listNonePrefix}a.json` },
    ]),
  }),
  customDefinitionCase({
    id: '013-listobjectsv2-load-and-flatten-maxitems-limits-materialized-records',
    title: 'ItemReader ListObjectsV2 with LOAD_AND_FLATTEN respects MaxItems',
    group,
    tags: ['observation', 'itemreader', 'load_and_flatten', 'maxitems', 'aws_only'],
    definition: {
      StartAt: 'ReadItems',
      States: {
        ReadItems: {
    ...
    expected: expectOutput([
      {
        Source: `s3://${itemReaderBucket}/${loadAndFlattenPrefix}one.json`,
        Value: { id: 'one-a' },
      },
      {
        Source: `s3://${itemReaderBucket}/${loadAndFlattenPrefix}one.json`,
